...
POLY       ?= poly
BIN        := bin
LIBDIR     := lib/github.com/sjqtentacles/sml-glm
...
example: $(BIN)/transforms
	mkdir -p assets
	./$(BIN)/transforms

$(BIN)/transforms: $(SRCS) examples/transforms.sml examples/sources.mlb | $(BIN)
...
# exits on its own, so we just `use` the sources in dependency order.
poly test-poly:
	printf 'use "$(LIBDIR)/glm.sig";\nuse "$(LIBDIR)/glm.sml";\nuse "test/harness.sml";\nuse "test/support.sml";\nuse "test/test_vec.sml";\nuse "test/test_mat.sml";\nuse "test/test_quat.sml";\nuse "test/entry.sml";\nuse "test/main.sml";\n' | $(POLY) -q --error-exit
